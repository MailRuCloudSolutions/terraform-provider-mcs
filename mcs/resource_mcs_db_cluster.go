	...
	return &schema.Resource{
		...
		Importer: &schema.ResourceImporter{
			State: func(d *schema.ResourceData, meta interface{}) ([]*schema.ResourceData, error) {
				...
				err = resourceDatabaseClusterRead(d, meta)
				if err != nil {
					return nil, err
				}
...
func resourceDatabaseClusterCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("error creating mcs_db_cluster: %s", err)
	}
...
func resourceDatabaseClusterRead(d *schema.ResourceData, meta interface{}) error {
	...
	if cluster.Instances[0].WalVolume.VolumeID != "" {
		...
		walVolumeFlattened := flattenDatabaseClusterWalVolume(*cluster.Instances[0].WalVolume)
...
func resourceDatabaseClusterUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("wal_disk_autoexpand") {
		...
		if err != nil {
			return fmt.Errorf("unable to determine mcs_db_cluster wal_disk_autoexpand")
		}
